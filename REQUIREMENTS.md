...
**Deviation from "ships with the OS, no new dependency":** links the system `libsqlite3` via `find_package(SQLite3 REQUIRED)` (desktop/Termux) and direct NDK linkage (`sqlite3`, Android) rather than vendoring an amalgamation, per the ticket's original intent — but Android's bundled SQLite only gained FTS5 support around API 30, while this app's floor is API 26 (NFR-014). Below API 30, `CREATE VIRTUAL TABLE ... USING fts5` is expected to fail and the code falls back to recency automatically (the same path already used for "no strong match"), so older devices degrade to FR-013's pre-existing behavior rather than crashing — un-verified on an actual low-API device (no emulator/device in this environment).
